...
LCD_LINE_3 = 0x14 # LCD RAM address for the 1st line
LCD_LINE_4 = 0x54 # LCD RAM address for the 2nd line
...
def lcd_initDisplay():
    # Initialise display
    lcd_byte(0x33,LCD_CMD) # wakeup call
    lcd_byte(0x32,LCD_CMD) # 4bit mode
    lcd_byte(0x28,LCD_CMD) # function set
    lcd_byte(0x0C,LCD_CMD) # Display on off control
    lcd_byte(0x06,LCD_CMD) # 
    lcd_byte(0x01,LCD_CMD) # clear display
    # Kurze wartezeit eingebaut, damit es zu keinem fehler kommt
    time.sleep(0.1)

def lcd_display(line1, line2, line3, line4, style):
    lcd_byte(LCD_LINE_1, LCD_CMD)
    lcd_string(line1, style)
    lcd_byte(LCD_LINE_2, LCD_CMD)
    lcd_string(line2, style)
    lcd_byte(LCD_LINE_3, LCD_CMD)
    lcd_string(line3, style)
    lcd_byte(LCD_LINE_4, LCD_CMD)
    lcd_string(line4, style)
